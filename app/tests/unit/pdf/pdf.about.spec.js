...
const { textMatcherFactory, createWrapper, propsMocker, propMocker } =
  myTest.default;
...
describe('PDFAbout.vue', () => {
  ...
  const propDicts = [
    ...
    { prop: 'citizenship', selector: '#pdf-citizenship' }
  ];
  let mockProp = true;
  ...
  propDicts.forEach(dict => {
    it(`renders interests ${dict.selector} that matches ${dict.prop} prop`, () => {
      expect(wrapper.find(`${dict.selector}`).text()).toMatch(
        propMocker(`${dict.prop}`).propsData[`${dict.prop}`]
      );
    });
  });
  ...
  it('email href matches email prop', () => {
    ...
    expect(wrapper.find(`${propDict.selector}`).attributes().href).toMatch(
      propMocker(`${prop}`).propsData[`${prop}`]
    );
  });
